...
var HanghoaViewModel = function () {
    ...
    self.luuGiaHanghoa = function (item) {
        try {
            ...
            // Kiểm tra xem code có trùng không? 
            var codeTrung = false;
            ko.utils.arrayForEach(self.ListGiaHanghoa(), function (val) {
                if (val.Code() == item.Code() && val != item) {
                    tenHanghoa = null;
                    codeHanghoa = null;
                    // Lấy tên hàng hóa + Code của hàng hóa bị trùng.
                    ko.utils.arrayForEach(self.List(), function (hanghoa) {
                        if (hanghoa.HanghoaId() == val.HanghoaId()) {
                            tenHanghoa = hanghoa.Ten();
                            codeHanghoa = hanghoa.Code();
                            return;
                        }
                    });

                    alert("Code \"" + val.Code() + "\" đã tồn tại cho hàng hóa có tên \"" + tenHanghoa + "(Code: " + codeHanghoa + ")" + "\".");
                    codeTrung = true;
                    return;
                }
            });
